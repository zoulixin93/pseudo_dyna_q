...
#     'lstrip', 'rstrip', 'run_with', 't', 'to_type', 'transpose','write','uniq'
# ]
...
####### pipe line defined by zoulixin
@Pipe
def replace(iterable,old=" ",new="\t"):
    for item in iterable:
        yield str(item).replace(old,new)

@Pipe
def uniq(iterable):
    iterable = set(list(iterable))
    for item in iterable:
        yield item

@Pipe
def open_file(iterable):
    res = []
...
def split(iterable,separator="\t"):
    for item in iterable:
        yield item.split(separator)

@Pipe
def filtering(iterable,func=lambda x:x):
    for item in iterable:
        yield func(item)